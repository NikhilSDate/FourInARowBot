...
def add_game(first_player_id: str, second_player_id: str, guild_id: str, channel_id: str, rows: int, columns: int,
             winning_length: int, moves: list,
...
def get_stats(player_id: str, other_player_id=None, guild_id: str = None, channel_id: str = None,
    ...
    pipeline.extend([
        {
            "$project":
                {
                    "player_result": {
                        "$cond": [{"$eq": ["$first_player_id", player_id]}, "$result", {"$subtract": [0, "$result"]}]}
                }
        },
