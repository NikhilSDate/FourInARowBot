...
class MyHelp(MinimalHelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[Cog], List[Command]], /) -> None:
        ...
        embed.add_field(name='', value="In command help, angle brackets indicate a required parameter. "
                                       "Square brackets indicate an optional parameter. "
                                       "\nA value for an optional parameter, if given, indicates the default value for that parameter. "
                                       "For example, `rows=6` in the help for the `newgame` command means that the default value for the `rows` parameter is `6`. "
